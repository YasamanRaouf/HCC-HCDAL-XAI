...
def train_sae(X_train, encoding_dim=32):
    ...
    sae.fit(X_train, X_train, epochs=100, batch_size=16, verbose=0)
    return encoder
...
def train_dbn(X_train, y_train):
    rbm1 = BernoulliRBM(n_components=64, learning_rate=0.01, n_iter=50,
                        batch_size=16, random_state=42)
    rbm2 = BernoulliRBM(n_components=32, learning_rate=0.01, n_iter=50,
                        batch_size=16, random_state=42)
